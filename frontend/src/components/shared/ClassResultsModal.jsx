...
import { FiX, FiLoader, FiAward, FiClock, FiCheckCircle } from 'react-icons/fi';
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
          <div>
            <h3 className="font-semibold text-slate-900 text-sm">Kết quả cả lớp</h3>
            <p className="text-xs text-slate-400 mt-0.5 truncate max-w-[280px]">{exam.examName}</p>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-xs text-slate-400 border-b border-slate-100">
                    ...
                    <th className="text-left px-3 py-2.5 font-medium">Học sinh</th>
                    <th className="text-center px-3 py-2.5 font-medium">Điểm</th>
                    <th className="text-center px-3 py-2.5 font-medium hidden sm:table-cell">Đúng</th>
                    <th className="text-center px-3 py-2.5 font-medium hidden sm:table-cell">T.gian</th>
                    <th className="text-right px-5 py-2.5 font-medium hidden md:table-cell">Nộp lúc</th>
                  </tr>
                </thead>
                <tbody>
                  {results.map((r, i) => {
                    ...
                    return (
                      <tr key={r.username}
                        className={`border-b border-slate-50 transition-colors ${isMe ? 'bg-indigo-50/70' : 'hover:bg-slate-50'}`}>
                        ...
                        <td className="px-3 py-2.5">
                          ...
                          {isMe && <span className="ml-1.5 text-[10px] font-semibold text-indigo-500 bg-indigo-100 px-1.5 py-0.5 rounded-full">Bạn</span>}
                        </td>
        ...
        <div className="px-5 py-3 border-t border-slate-100 shrink-0">
          <button className="btn-secondary w-full py-1.5 text-sm" onClick={onClose}>Đóng</button>
        </div>
      </div>
    </div>,
    document.body
