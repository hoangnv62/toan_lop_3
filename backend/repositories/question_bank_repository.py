...
class QuestionBankRepository:
    def find_by_teacher(self, teacher_id: int, q: str = "", page: int = 1, limit: int = 10, lesson_id=None) -> dict:
        offset = (page - 1) * limit
        ...
        if lesson_id == 0:
            base = base.where(QuestionBankQuestion.lesson_id == None)
        elif lesson_id is not None:
            base = base.where(QuestionBankQuestion.lesson_id == lesson_id)
    ...
    def create(self, teacher_id: int, content: str, explanation, answers_data: list, lesson_id=None) -> QuestionBankQuestion:
        q = QuestionBankQuestion(teacher_id=teacher_id, content=content, explanation=explanation, lesson_id=lesson_id)
        db.session.add(q)
        db.session.flush()
    ...
    def update(self, q: QuestionBankQuestion, content: str, explanation, answers_data: list, lesson_id=None) -> None:
        q.content     = content
    ...
    def bulk_create(self, teacher_id: int, questions_data: list, lesson_id=None) -> int:
        count = 0
