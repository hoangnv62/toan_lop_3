...
export const getTeacherDashboard = async (teacherId) => {
  const summary = await queryOne(
    `SELECT
       (SELECT COUNT(*) FROM users u JOIN classes c ON u.class_id=c.id WHERE c.teacher_id=:tid AND u.role='student') AS totalStudents,
       ...
       (SELECT COUNT(*) FROM exams e JOIN lessons l ON e.lesson_id=l.id WHERE l.teacher_id=:tid) AS totalExams`,
    { tid: teacherId }
  );
  ...
  for (const s of scoreRows) {
    ...
    if (v < 4)      { dist['0-4']++; failCount++; }
    else if (v < 6) { dist['4-6']++; failCount++; }
    else if (v < 8) { dist['6-8']++; passCount++; }
    else            { dist['8-10']++; passCount++; }
  ...
  const classAvgRows = await query(
    `SELECT c.class_name AS className, ROUND(AVG(t.student_avg), 2) AS avgScore
     ...
     GROUP BY c.id, c.class_name ORDER BY avgScore DESC`,
    { tid: teacherId }
  );
