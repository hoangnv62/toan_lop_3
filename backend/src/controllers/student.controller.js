...
export const searchStudents = async (req, res) => {
  ...
  res.json({ success: true, data: await svc.searchStudents(q, classId) });
};

export const addToClass = async (req, res) => {
  const { username } = req.body;
  const fullName = await svc.addToClass(parseInt(req.params.classId), req.user.user_id, username.trim());
  res.json({ success: true, message: `Đã thêm ${fullName} vào lớp` });
};

export const uploadStudents = async (req, res) => {
  if (!req.file) return res.status(400).json({ success: false, message: 'Không tìm thấy file' });
  const result = await svc.uploadStudents(parseInt(req.params.classId), req.user.user_id, req.file.buffer);
  ...
  res.json({ success: true, message: msg, count: result.count });
};

export const removeFromClass = async (req, res) => {
  await svc.removeFromClass(parseInt(req.params.classId), req.user.user_id, parseInt(req.params.studentId));
  res.json({ success: true, message: 'Đã gỡ học sinh khỏi lớp' });
};

export const getStudentResults = async (req, res) => {
  res.json({ success: true, data: await svc.getStudentResults(parseInt(req.params.id)) });
};

export const getStudentProgress = async (req, res) => {
  res.json({ success: true, data: await svc.getStudentProgress(parseInt(req.params.id)) });
};
...
export const studentDashboard = async (req, res) => {
  ...
  res.json({ success: true, data: await svc.getStudentDashboard(req.user.user_id, showAll, dateFrom, dateTo) });
};
